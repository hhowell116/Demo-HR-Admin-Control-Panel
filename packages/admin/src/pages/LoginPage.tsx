import { useAuth } from '../auth/AuthProvider';

export function LoginPage() {
  const { signIn, error, loading } = useAuth();

  return (
    <div className="min-h-screen bg-brand-off-white flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <div className="w-16 h-16 rounded-full bg-gradient-to-br from-brand-warm-brown to-brand-taupe flex items-center justify-center mx-auto mb-4">
            <span className="text-white text-xl font-bold font-serif">RC</span>
          </div>
          <h1 className="text-2xl font-serif font-bold text-brand-deep-brown">
            Acme Co
          </h1>
          <p className="text-sm text-brand-taupe mt-1">HR TV Admin</p>
        </div>

        <div className="bg-white rounded-xl border border-brand-border p-6 shadow-sm">
          {error && (
            <div className="mb-4 p-3 rounded-lg bg-red-50 border border-red-200 text-sm text-red-700">
              {error}
            </div>
          )}

          <button
            onClick={signIn}
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-lg
              bg-brand-warm-brown text-white font-medium text-sm
              hover:bg-brand-deep-brown transition-colors disabled:opacity-50"
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#fff" />
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#fff" opacity="0.8" />
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#fff" opacity="0.6" />
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#fff" opacity="0.9" />
            </svg>
            Sign in with Google
          </button>

          <p className="text-[11px] text-center text-brand-taupe mt-4">
            Restricted to authorized accounts
          </p>
        </div>
      </div>
    </div>
  );
}
