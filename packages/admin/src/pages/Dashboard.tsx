...
export function Dashboard() {
  ...
  const stats = useMemo<StatCard[]>(() => {
    ...
    const activeEmps = DEMO_EMPLOYEES.filter((e) => e.isActive);

    const birthdays = activeEmps.filter((e) => e.birthMonth === currentMonth).length;
    const anniversaries = activeEmps.filter((e) => {
      if (!e.hireDate) return false;
      const hire = new Date(e.hireDate as string);
      return hire.getMonth() + 1 === currentMonth;
    }).length;
    const activeCampaigns = DEMO_CAMPAIGNS.filter((c) => c.isActive).length;

    return [
      { label: 'Active Employees', value: activeEmps.length, icon: Users, color: 'bg-brand-warm-brown' },
      ...
      { label: 'Active Campaigns', value: activeCampaigns, icon: Trophy, color: 'bg-brand-taupe' },
    ];
  }, []);
